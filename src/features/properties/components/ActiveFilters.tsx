...
export function ActiveFilters({ listingType, type, minPrice, maxPrice, minBedrooms, sortBy, sortOrder }: FilterParams) {
  const router = useRouter()

  const active = CHIP_KEYS.filter((k) => {
    const vals: Record<ChipKey, string | undefined> = { listingType, type, minPrice, maxPrice, minBedrooms }
    return !!vals[k]
  })
  ...
  const allParams: Record<ChipKey | "sortBy" | "sortOrder", string | undefined> = {
    listingType, type, minPrice, maxPrice, minBedrooms, sortBy, sortOrder,
  }
    ...
    <div className="mb-4 flex flex-wrap items-center gap-2">
      {active.map((key) => {
        ...
        return (
          <span
            key={key}
            className="inline-flex items-center gap-1.5 rounded-full bg-[var(--navy)]/10 px-3 py-1 text-xs font-medium text-[var(--navy)] dark:bg-[var(--navy)]/30 dark:text-blue-300"
          >
            {CHIP_LABEL[key](val)}
            <button
              type="button"
